...
class BaseAviary(gym.Env):
    ...
    def _apply_waveforce_to_platform(
        ...
        # bound, samples = 1.5, 3
        bound = 1.5
        disp = np.zeros([2], dtype=np.single)
        ...
        coord = np.array([[bound, 0], [0, bound]])
        for wave in waves:
            disp += np.clip(
                (eval_wave(wd=wave, timestep=timestep, coord=coord)),
                a_min=None,
                a_max=None,
            )
    ...
    def _setupLandingPad(self) -> int:
        ...
        match self.DIFFICULTY:
            case Difficulty.SS0:
                ...
                self.WAVES = []
            case Difficulty.SS1:
                self.LANDPAD_SPRING_CONSTANT = 4
                # Motor position control with limited force gives is a restoration force back to neutral position.
                # Up-down restoration force
                p.setJointMotorControl2(
                    objIdx, 0, p.POSITION_CONTROL, targetPosition=0, force=100
                ...
                self.WAVES = [
                    WaveData(wl=16, amp=0.2, ws=0.4, theta=1.8),
                    WaveData(wl=12, amp=0.2, ws=0.4, theta=0),
                ]
            case Difficulty.SS3:
    ...
    def _exportImage(
        ...
        if img_type == ImageType.RGB:
            ...
            raise RuntimeError()
        if img_type != ImageType.RGB and temp is not None:
